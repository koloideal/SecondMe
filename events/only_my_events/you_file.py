    ...
    if event.is_private:
        ...
        if not avatar:
            await client.send_message(
                chat, caption, parse_mode="HTML", reply_to=event.message
            )

        else:
            await client.send_file(
                chat, avatar, caption=caption, parse_mode="HTML", reply_to=event.message
            )
