    ...
    if event.is_reply:
        ...
        if replied_msg.text:
            if len(replied_msg.text) < 4 or len(replied_msg.text) > 20:
                await event.reply(
                    "<code>The message should be 4 < len < 20 characters</code>",
                    parse_mode="HTML",
                )
            ...
            if not avatar:
                ...
                await event.reply(
                    "<code>User must have an avatar</code>", parse_mode="HTML"
                )
            ...
            uploaded_media = await client(
                UploadMediaRequest(
                    ...
                    media=InputMediaUploadedDocument(
                        ...
                        mime_type="image/png",
                        attributes=[
                            DocumentAttributeFilename("sticker_data/sticker.png")
                        ],
                    ),
                )
            )
            ...
            await event.reply(
                "<code>Usage: [reply to a text message]</code>", parse_mode="HTML"
            )
    else:
        await event.reply(
            "<code>Usage: [reply to a text message]</code>", parse_mode="HTML"
