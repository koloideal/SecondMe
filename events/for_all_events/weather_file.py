...
async def weather(event, client):
    ...
    if len(event.message.text.split()) == 1:
        response = requests.get(
            f"https://api.openweathermap.org/data/2.5/weather?q=Minsk&appid={api_key}&units=metric"
        )
    ...
    if response.status_code == 200:
        ...
        await client.send_message(
            chat, "<pre>Invalid city</pre>", parse_mode="HTML", reply_to=event.message
        )
